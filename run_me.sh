...
avrdude -p m32u4 -c $ISP_PROGRAMMER -P $ISP_TTYPORT -b $ISP_SPEED -U flash:w:MassStorage-v3.0.5.hex
...
avrdude -p m1284p -c $ISP_PROGRAMMER -P $ISP_TTYPORT -b $ISP_SPEED -U flash:w:optiboot_atmega1284p_8MHz.hex
...
avrdude -p m1284p -b $SERIAL_SPEED -c $SERIAL_PROGRAMMER -P $SERIAL_TTYPORT -U flash:w:bGeigie3-v3.0.5-iRover-JP.hex
...
python bGeigie_QC.py -p $SERIAL_TTYPORT -b 57600
